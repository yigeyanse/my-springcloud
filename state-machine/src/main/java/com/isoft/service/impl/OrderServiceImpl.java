...
public class OrderServiceImpl implements OrderService {
    ...
    public Order pay(int id) {
        ...
        //logger.warn("threadName=" + Thread.currentThread().getName() + " 尝试支付 id=" + id);
        logger.warn(" 尝试支付 id=" + id);
        Message message = MessageBuilder.withPayload(OrderStatusChangeEvent.PAYED).setHeader("order", order).build();
        if (!sendEvent(message, order)) {
            logger.warn("threadName=" + Thread.currentThread().getName() + " 支付失败, 状态异常 id=" + id);
        }
